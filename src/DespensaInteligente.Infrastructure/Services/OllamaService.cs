...
namespace DespensaInteligente.Infrastructure.Services
{
    public class OllamaService : ILlmService
    {
        ...
        {
            _logger.LogInformation("Iniciando extração de dados da Nota Fiscal com o provedor Ollama. Modelo: {Model}", _options.Model);
            ...
            try
            {
                ...
                _logger.LogInformation("Chamada ao Ollama realizada com sucesso em {ElapsedMs}ms.", stopwatch.ElapsedMilliseconds);
            ...
            {
                _logger.LogError(ex, "Falha na extração de dados da Nota Fiscal no provedor Ollama.");
                throw new InvalidLlmResponseException("Falha ao obter uma resposta válida do Ollama.", ex);
            }
        }
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                var request = new HttpRequestMessage(HttpMethod.Post, "http://localhost:11434/v1/chat/completions");
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    _logger.LogError("Erro do provedor Ollama: {StatusCode} - {ErrorContent}", statusCode, errorContent);
                    throw new InvalidLlmResponseException($"Provedor Ollama retornou erro HTTP: {statusCode} - {errorContent}");
                }

                var responseData = await response.Content.ReadFromJsonAsync<OllamaResponseDto>();
                ...
                {
                    throw new InvalidLlmResponseException("A resposta obtida do Ollama está vazia ou malformada.");
                }
            ...
            {
                _logger.LogError(ex, "Timeout na chamada ao Ollama.");
                throw new LlmTimeoutException("A requisição ao provedor Ollama expirou.", ex);
            }
            catch (LlmException)
            {
                throw;
            ...
            {
                _logger.LogError(ex, "Erro de comunicação com o Ollama.");
                throw new InvalidLlmResponseException("Erro inesperado na chamada ao Ollama.", ex);
            }
        }
        ...
        private InvoiceExtractionResult ParseLlmResponse(string rawJson)
        {
            ...
            catch (JsonException ex)
            {
                ...
                throw new InvalidJsonException("Resposta do Ollama não contém um JSON estruturado válido.", ex);
            }
            catch (Exception ex) when (ex is not LlmException)
            {
                _logger.LogError(ex, "Erro inesperado ao tratar e normalizar dados do Ollama.");
                throw new InvalidLlmResponseException("Falha ao analisar os dados retornados pelo Ollama.", ex);
            }
        }
        ...
        private class OllamaResponseDto
        {
